...
class webmdSkill(MycroftSkill):
    def __init__(self):
        super(webmdSkill, self).__init__(name="webmdSkill")

    @intent_handler(IntentBuilder("").require("webmd").
    ...
    @intent_handler(IntentBuilder("").require("More").
                    require("web_article").require("spoken_lines"))
    def handle_tell_more(self, message):
        # Read more of the last article queried
        article = message.data.get("web_article")
        lines_spoken_already = int(message.data.get("spoken_lines"))

        summary_read = web.summary(article, lines_spoken_already)
        summary = web.summary(article, lines_spoken_already+5)
        ...
        if not summary:
            ...
            self.speak(summary)
            self.set_context("web_article", article)
    ...
    def handle_random_intent(self, message):
        ...
        self._lookup(web.random(pages=1))

    def _lookup(self, search):
        try:
            # Use the version of Wikipedia appropriate to the request language
            dict = self.translate_namedvalues("webmd_lang")
            web.set_lang(dict["code"])
            ...
            # is all we ever need.
            lines = 2
            summary = web.summary(results[0], lines)
            ...
            self.set_context("web_article", results[0])
